...
MUTANTS: tuple[Mutant, ...] = (
    ...
    Mutant(
        label="the thin-cell guard is one-sided again, so an empty active cohort ships a rate",
        decision="D20",
        path=f"{ENGINE}/corpus/contamination.py",
        old="        return min(self.active_restatable, self.dormant_restatable) >= MIN_",
        new="        return max(self.active_restatable, self.dormant_restatable) >= MIN_",
        tests=(f"{TESTS}/test_contamination.py",),
    ),
    Mutant(
        label="the card serialises a suppressed conditional rate anyway (the shipped defect)",
        decision="D20",
        path=f"{ENGINE}/corpus/contamination.py",
        old="        if self.conditional_is_reportable:",
        new="        if True:",
        tests=(f"{TESTS}/test_contamination.py",),
    ),
    Mutant(
        label="the pooled conditional gap ships without the sentence saying it is not a finding",
        decision="D20",
        path=f"{ENGINE}/corpus/contamination.py",
        old='                "conditional_caveat": (\n                    "NOT A FINDING.',
        new='                "conditional_caveat": (\n                    "A finding.',
        tests=(f"{TESTS}/test_contamination.py",),
    ),
    Mutant(
    ...
    Mutant(
        ...
        decision="D20",
        path=f"{ENGINE}/corpus/contamination.py",
        old="        return self.dormant_share - self.active_share",
        new="        return self.active_share - self.dormant_share",
        tests=(f"{TESTS}/test_contamination.py",),
    ),
    Mutant(
        label="the survival split reports one cutoff, making it a choice after the fact",
        decision="D20",
        path=f"{ENGINE}/corpus/contamination.py",
        old="    for cutoff in SURVIVAL_CUTOFFS:",
        new="    for cutoff in SURVIVAL_CUTOFFS[:1]:",
        tests=(f"{TESTS}/test_contamination.py",),
    ),
    Mutant(
        label="the survival cohorts join to filings again, dropping facts with no filing row",
        decision="D20",
        path=f"{ENGINE}/corpus/contamination.py",
        old="    SELECT cik, max(filed_at) AS last_filed_at FROM v_facts_pit GROUP BY cik",
        new="    SELECT cik, max(filed_at) AS last_filed_at FROM filings GROUP BY cik",
        tests=(f"{TESTS}/test_contamination.py",),
    ),
    Mutant(
