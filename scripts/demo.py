    ...
    with Application.build(data_dir=args.data_dir) as app:
        ...
        if outcome.failed:
            ...
            if "rate limit" in reason.lower() or "threshold" in reason.lower():
                print("\n     EDGAR served its 403 refusal page. That page is the SAME for two")
                print("     different causes and the body does not distinguish them:")
                print()
                print("       1. A genuine throttle. Clears on its own, usually within about")
                print("          ten minutes. Re-run then -- this script is idempotent and")
                print("          will pick up where it stopped.")
                print("       2. A User-Agent the SEC will not accept. This NEVER clears, and")
                print("          re-running forever will not help.")
                print()
                print(f"     Current ALETHEIA_SEC_USER_AGENT: {app.settings.sec_user_agent!r}")
                print()
                print("     Check which one you have before waiting. This returns 200 on a")
                print("     healthy address and 403 on a rejected one:")
                print()
                print('       curl -s -o /dev/null -w "%{http_code}\\n" \\')
                print(f'            -A "{app.settings.sec_user_agent}" \\')
                print("            https://www.sec.gov/files/company_tickers.json")
                print()
                print("     If that prints 403 immediately, it is cause 2 -- change the")
                print("     address. Observed rejected: any e-mail at a github.com domain,")
                print("     including users.noreply.github.com. A plain address at your own")
                print("     domain works.")
            else:
